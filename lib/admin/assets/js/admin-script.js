(function($){
    ...
    $(document).ready(function () {
        ...
        // ============================================
        // Post Type & Taxonomy Dynamic Reload
        // ============================================
        var lmaTaxTermsCache = {};

        $(document).on('change', '#lma-post-type', function() {
            var postType = $(this).val();
            loadTaxonomiesAndTerms(postType, true);
        });

        $(document).on('change', '#lma-taxonomy', function() {
            var taxonomy = $(this).val();
            var postType = $('#lma-post-type').val();
            var cacheKey = postType;
            if (lmaTaxTermsCache[cacheKey]) {
                renderTermCheckboxes(lmaTaxTermsCache[cacheKey].terms[taxonomy] || []);
            }
            updatePreview();
        });

        function loadTaxonomiesAndTerms(postType, resetTaxonomy) {
            if (lmaTaxTermsCache[postType]) {
                populateFromCache(postType, resetTaxonomy);
                return;
            }

            $.post(lmaAdmin.ajaxUrl, {
                action: 'lma_get_taxonomies_and_terms',
                post_type: postType,
                nonce: lmaAdmin.nonce
            }, function(response) {
                if (response.success) {
                    lmaTaxTermsCache[postType] = response.data;
                    populateFromCache(postType, resetTaxonomy);
                }
            });
        }

        function populateFromCache(postType, resetTaxonomy) {
            var data = lmaTaxTermsCache[postType];

            // Populate taxonomy dropdown
            var $taxSelect = $('#lma-taxonomy');
            $taxSelect.empty();
            $.each(data.taxonomies, function(i, tax) {
                $taxSelect.append($('<option>').val(tax.slug).text(tax.label));
            });

            if (resetTaxonomy && data.taxonomies.length > 0) {
                $taxSelect.val(data.taxonomies[0].slug);
            }

            // Populate term checkboxes
            var selectedTax = $taxSelect.val();
            renderTermCheckboxes(data.terms[selectedTax] || []);
            updatePreview();
        }

        function renderTermCheckboxes(terms) {
            var includeHtml = '';
            var excludeHtml = '';

            if (terms.length === 0) {
                var noTerms = '<p class="description">No terms found.</p>';
                $('#lma-include-cats').html(noTerms);
                $('#lma-exclude-cats').html(noTerms);
            } else {
                $.each(terms, function(i, term) {
                    var safeName = $('<span>').text(term.name).html();
                    var checkbox = '<label><input type="checkbox" name="{name}[]" value="' + term.term_id + '"> ' +
                        safeName + ' <span class="lma-cat-count">(' + term.count + ')</span></label>';
                    includeHtml += checkbox.replace('{name}', 'include_cats');
                    excludeHtml += checkbox.replace('{name}', 'exclude_cats');
                });
                $('#lma-include-cats').html(includeHtml);
                $('#lma-exclude-cats').html(excludeHtml);
            }
        ...
        function getFormValues() {
            return {
                ...
                postType: $('#lma-post-type').val() || 'post',
                taxonomy: $('#lma-taxonomy').val() || '',
            };
        }
        ...
        function buildShortcode(vals) {
            ...
            sc += ' post_type="' + vals.postType + '"';
            if (vals.taxonomy) sc += ' taxonomy="' + vals.taxonomy + '"';
            sc += ' style="' + vals.style + '"';
