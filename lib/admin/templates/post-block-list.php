<?php if (!defined('ABSPATH'))
    exit; ?>
<div class="wrap">
    <h1 class="wp-heading-inline"><?php esc_html_e('Post Block Shortcode', 'load-more-ajax'); ?></h1>
    <a href="<?php echo esc_url(admin_url('admin.php?page=load_more_ajax&action=new')); ?>" class="page-title-action">Add New</a>
    ...
    <table class="wp-list-table widefat fixed striped table-view-list posts">
        ...
        <thead>
            <tr>
                ...
                <th scope="col" id="date" class="manage-column column-date sorted desc" aria-sort="descending" abbr="Date"><a
        ...
        <tbody id="the-list">
            ...
            if (!empty($all_blocks)) {
                foreach ($all_blocks as $data) {
                    ...
                    $block_style = 'style="' . esc_attr($data['block_style']) . '" ';
                    $per_page = !empty($data['per_page']) ? 'posts_per_page="' . esc_attr($data['per_page']) . '" ' : '';
                    $filter = !empty($data['is_filter']) ? 'filter="true" ' : '';
                    $include = !empty($data['include_post']) ? 'include="' . esc_attr($data['include_post']) . '" ' : '';
                    $exclude = !empty($data['exclude_post']) ? 'exclude="' . esc_attr($data['exclude_post']) . '" ' : '';
                    $text_limit = !empty($data['text_limit']) ? 'text_limit="' . esc_attr($data['text_limit']) . '" ' : '';
                    ...
                    $cloumn = !empty($data['post_column']) ? 'column="' . esc_attr($col_val) . '" ' : ''; ?>
                    ...
                    <tr id="post-1"
                        ...
                        <td class="title column-title has-row-actions column-primary page-title" data-colname="Title">
                            ...
                            <strong><a class="row-title"
                                    href="?page=load_more_ajax&action=edit&post_block=<?php echo esc_attr($data['id']) ?>"
                                    aria-label=""><?php echo esc_html($data['block_title']) ?></a></strong>
                        </td>
                        <td class="column-post-type" data-colname="Post Type">
                            <span class="lma-post-type-badge"><?php echo esc_html($pt_value); ?></span>
                        </td>
                        <td class="column-shortcode" data-colname="shortcode"><a href="javascript:void(0)"
                                class="copy_block_shortcode">[load_more_ajax_lite
                                <?php
                                // All shortcode attribute values are escaped individually above via esc_attr().
                                echo $post_type . $taxonomy_attr . $block_style . $per_page . $filter . $include . $exclude . $text_limit . $cloumn // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                                ?>]</a>
                        </td>
                        <td class="author column-author" data-colname="Author"><a
                                href="edit.php?post_type=post&amp;author=1">ajanta</a></td>
        ...
        <tfoot>
            <tr>
                ...
                <th scope="col" class="manage-column column-title column-primary sortable desc" abbr="Title"><a
                        href="#"><span>Title</span><span
                            class="sorting-indicators"><span class="sorting-indicator asc"
                                aria-hidden="true"></span><span class="sorting-indicator desc"
                                aria-hidden="true"></span></span> <span class="screen-reader-text">Sort
                            ascending.</span></a></th>
                ...
                <th scope="col" class="manage-column column-date sorted desc" aria-sort="descending" abbr="Date"><a
                        href="#"><span>Date</span><span
                            class="sorting-indicators"><span class="sorting-indicator asc"
                                aria-hidden="true"></span><span class="sorting-indicator desc"
                                aria-hidden="true"></span></span></a></th>
