...
function load_more_ajax_lite_estimated_reading_time($post_id)
{
    ...
    $sec = $second . esc_html__(' sec', 'load-more-ajax');

    $estimate = 1 > $minute ? $sec : $min;
    $output = $estimate . esc_html__(' read', 'load-more-ajax');
...
function load_more_ajax_lite_with_cat_filter()
{
    try {
        ...
        if (class_exists('LMA_Security')) {
            ...
            if (!$posttype) {
                wp_send_json_error(['error' => true, 'message' => esc_html__('Invalid post type.', 'load-more-ajax')]);
            }
        ...
        if (!$order) {
            wp_send_json_error(['error' => true, 'message' => esc_html__('Invalid request parameters.', 'load-more-ajax')]);
        }
    ...
    try {
        ...
        if (class_exists('LMA_Security')) {
            if (!LMA_Security::verify_ajax_nonce('load_more_ajax_nonce', 'nonce')) {
                wp_send_json_error(['message' => esc_html__('Security check failed.', 'load-more-ajax')]);
            }
        ...
        wp_send_json_error(['message' => esc_html__('Could not get post count.', 'load-more-ajax')]);
    }
}
    ...
    try {
        ...
        if (class_exists('LMA_Security')) {
            ...
            if (!LMA_Security::check_rate_limit('search_posts', 30)) {
                wp_send_json_error(['message' => esc_html__('Too many search requests.', 'load-more-ajax')]);
            }
        ...
        if (strlen($search_term) < 3) {
            wp_send_json_error(['message' => esc_html__('Search term must be at least 3 characters.', 'load-more-ajax')]);
        }
        ...
        wp_send_json_error(['message' => esc_html__('Search failed.', 'load-more-ajax')]);
    }
}
